...
- (void)configCollectionView {
    ...
    _collectionView = [[TZCollectionView alloc] initWithFrame:CGRectMake(0, 42, self.view.tz_width, [UIScreen mainScreen].bounds.size.height) collectionViewLayout:layout];
...
- (void)configBottomToolBar {
    ...
    UIView *bottomToolBar = [[UIView alloc] initWithFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height, self.view.tz_width, 50)];
...
- (UIView *)topView {
    if (!_topView) {
        _topView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 42)];
...
- (UIButton *)cancelBtn {
    if (!_cancelBtn) {
        ...
        _cancelBtn.frame = CGRectMake(10, 20, 40, 20);
...
- (UIButton *)continueBtn {
    if (!_continueBtn) {
        ...
        [_continueBtn addTarget:self action:@selector(doneButtonClick) forControlEvents:UIControlEventTouchUpInside];
    }
    return _continueBtn;
}
